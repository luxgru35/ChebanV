...
router.post('/register', async (req, res) => {
    try {
        ...
        // Generate JWT token
        const token = (0, jsonwebtoken_1.sign)({ id: user.id, email: user.email }, process.env.JWT_SECRET, { expiresIn: process.env.JWT_EXPIRES_IN || '1h' });
        res.status(201).json({
...
router.post('/login', async (req, res) => {
    try {
        ...
        const recentLogins = await LoginHistory_1.default.findAll({
            where: { userId: user.id },
            order: [['loginAt', 'DESC']],
            limit: 10, // Check last 10 logins
        });
        const isNewDevice = !recentLogins.some((login) => login.ipAddress === ipAddress && login.userAgent === userAgent);
        // Save login history
        ...
        if (isNewDevice && recentLogins.length > 0) {
            ...
            (0, emailService_1.sendNewDeviceAlert)(user.email, user.name, ipAddress, userAgent, new Date())
